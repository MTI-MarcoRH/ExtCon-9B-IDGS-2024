...
-- Elaborado por: Jesus Flores Valencia 
-- Grado y Grupo: 9B
...
-- b) Cambios sugeridos
-- se elimino el campo id_empleado y la llave foranea ya que esta no son necesarios en la tabla 
CREATE TABLE `tbd_servicios_al_cliente` (
  `Id` int unsigned NOT NULL AUTO_INCREMENT,
  `Id_persona` int unsigned NOT NULL,
  `tipo_servicio` enum('Consulta','Reclamo','Sugerencia') NOT NULL,
...
-- esta tabla no contaba con el procedimiento (se creo procedimeto con poblacion estatica)
CREATE DEFINER=`adan`@`localhost` PROCEDURE `sp_poblar_servicios_al_clientes`(v_password varchar(10))
BEGIN
if v_password = '12345' then

    INSERT INTO tbd_servicios_al_cliente VALUES
    (DEFAULT,1, 'Consulta', 'Consulta inicial sobre los servicios del gimnasio', 'Cliente interesado en saber los horarios disponibles ', default, default, default),
    (DEFAULT,1, 'Consulta', 'Consulta sobre como hacer buen uso de las maquinas', 'Cliente muy satisfecho con el sertvicio actual ', default, default, default),
	 (DEFAULT,3, 'Sugerencia', 'Sugerencia para agregar mas equipo para trabajar los musculos de las piernas', 'Cliente muy satisfecho con el servicio actual', default, default, default),
     (DEFAULT,4, 'Consulta', 'Reclamo sobre el mal funcionamiento de una máquina', 'Cliente pide reparación urgente', default, default, default),
     (DEFAULT,5, 'Sugerencia', 'Poner musica en el gimanasio para motivacion', 'Cliente muy satisfecho con el servicio actual', default, default, default);
     
      
	update tbd_servicios_al_cliente set Tipo_Servicio = 'Reclamo' where id = 4;
	update tbd_servicios_al_cliente set Estatus = b'0' where id = 2;
        
        delete from tbd_servicios_al_cliente where id = 1;
		else
...
-------------Trigger para insertar en la bitacora correcto-------------------------------
...
-------------Trigger para actualizar la fecha de actualizaciion de la tabla  en la bitacora correcto-------------------------------
CREATE DEFINER=`adan`@`%` TRIGGER `tbd_servicios_al_cliente_BEFORE_UPDATE` BEFORE UPDATE ON `tbd_servicios_al_cliente` FOR EACH ROW BEGIN
SET new.fecha_actualizacion = current_timestamp();
END


-- e) Realizar una consulta join (en caso de que aplique) para comprobar la integridad de la información.
-- no aplica realizar el join 
...
-- Tabla servicios sucursales
    ...
    CREATE DEFINER=`adan`@`localhost` PROCEDURE `sp_poblar_servicios_sucursales`(v_password varchar(10))
        BEGIN
    ...
    CREATE DEFINER=`adan`@`localhost` TRIGGER `tbd_servicios_sucursales_AFTER_INSERT` AFTER INSERT ON `tbd_servicios_sucursales` FOR EACH ROW BEGIN
        DECLARE v_estatus VARCHAR(20) DEFAULT 'Activo';
        ...
        END
    -- ------------------------------------------------------------------------------------------------------
    CREATE DEFINER=`adan`@`localhost` TRIGGER `tbd_servicios_sucursales_AFTER_UPDATE` AFTER UPDATE ON `tbd_servicios_sucursales` FOR EACH ROW BEGIN
        ...
        END
    -- ------------------------------------------------------------------------------------------------------
    CREATE DEFINER=`adan`@`localhost` TRIGGER `tbd_servicios_sucursales_BEFORE_DELETE` BEFORE DELETE ON `tbd_servicios_sucursales` FOR EACH ROW BEGIN
        DECLARE v_estatus_old VARCHAR(20) DEFAULT 'Activo';
...
-- Tabla instalaciones
        ...
        -- b) cambios sugeridos
        -- c) Revision de la poblacion estatica (correccion en caso de ser necesaria)
        CREATE DEFINER=`valencia`@`localhost` PROCEDURE `sp_poblar_instalaciones`(v_password varchar(10))
            BEGIN

            if v_password = '1234' then

                INSERT INTO tbb_instalaciones VALUES
                (Default, 'Área de CrossFit', "Espacio equipado para CrossFit", "Entrenamiento", '2020-08-10 08:00:00', default, default, default,1,"Lunes a Viernes de 8:00am a 14:00pm", null, null),
                (Default, 'Sala de Aeróbicos', "Espacio para clases de aeróbicos y danza fitness", "Cardio",'2020-08-10 08:00:00', default, default, default,1,"Lunes a Viernes de 8:00am a 8:00pm", null, null),
            ...
            END
        -- d) Revision de los 4 triggers (AFTER INSERT, BEFORE UPDATE, AFTER UPDATE, BEFORE DELETE)
        CREATE DEFINER=`valencia`@`localhost` TRIGGER `tbb_instalaciones_AFTER_INSERT` AFTER INSERT ON `tbb_instalaciones` FOR EACH ROW BEGIN
            DECLARE v_estatus VARCHAR(20) DEFAULT 'Activo';
        ...
        CREATE DEFINER=`valencia`@`%` TRIGGER `tbd_instalacion_BEFORE_UPDATE` BEFORE UPDATE ON `tbb_instalaciones` FOR EACH ROW BEGIN
            ...
            END
        -- -------------------------------------------------------------------------------------------------------------
        CREATE DEFINER=`valencia`@`localhost` TRIGGER `tbb_instalaciones_AFTER_UPDATE` AFTER UPDATE ON `tbb_instalaciones` FOR EACH ROW BEGIN
            DECLARE v_estatus_old VARCHAR(20) DEFAULT 'Activo';
            DECLARE v_estatus_new VARCHAR(20) DEFAULT 'Activo';
            ...
            END
        -- -------------------------------------------------------------------------------------------------------------
        CREATE DEFINER=`valencia`@`localhost` TRIGGER `tbb_instalaciones_BEFORE_DELETE` BEFORE DELETE ON `tbb_instalaciones` FOR EACH ROW BEGIN
            DECLARE v_estatus_old VARCHAR(20) DEFAULT 'Activo';
    ...
    -- b) cambios sugeridos
    -- c) Revision de la poblacion estatica (correccion en caso de ser necesaria)
    CREATE DEFINER=`zahid`@`localhost` PROCEDURE `sp_poblar_equipamientos`(v_password varchar(10))
        BEGIN
        ...
        END
    -- d) Revision de los 4 triggers (AFTER INSERT, BEFORE UPDATE, AFTER UPDATE, BEFORE DELETE)
    CREATE DEFINER=`zahid`@`localhost` TRIGGER `tbb_equipamientos_AFTER_INSERT` AFTER INSERT ON `tbb_equipamientos` FOR EACH ROW BEGIN
        ...
        END
    -- ----------------------------------------------------------------------------------------------------
    CREATE DEFINER=`zahid`@`localhost` TRIGGER `tbb_equipamientos_BEFORE_UPDATE` BEFORE UPDATE ON `tbb_equipamientos` FOR EACH ROW BEGIN
        set new.Fecha_Actualizacion = current_timestamp();
        END
    -- ----------------------------------------------------------------------------------------------------
    CREATE DEFINER=`zahid`@`localhost` TRIGGER `tbb_equipamientos_AFTER_UPDATE` AFTER UPDATE ON `tbb_equipamientos` FOR EACH ROW BEGIN
        declare v_estatus_old varchar(20) default 'Activo';
        declare v_estatus_new varchar(20) default 'Activo';
        ...
        END
    -- --------------------------------------------------------------------------------------------------------
    CREATE DEFINER=`zahid`@`localhost` TRIGGER `tbb_equipamientos_BEFORE_DELETE` BEFORE DELETE ON `tbb_equipamientos` FOR EACH ROW BEGIN
        declare v_estatus_old varchar(20) default 'Activo';
